#' Return the pediatric coefficient when H_0 holds 
#' 
#' This function will give corresponding  pediatric coefficient for eta=0.1,0.15,0.25,...,0.95.
#' 
...
#' @return A matrix containing all the pediatric coefficients with respect to eta=0.1,0.15,0.25,...,0.95.


library(nleqslv)
library(dplyr)

typeI_coeff <- function(eps_H_seq, n_adu, lower, upper, beta_adu) {
  ...
  for (eps_H in eps_H_seq) {
    y_resp <- c(max(fit(lower, beta_adu) - eps_H, 0), fit(upper, beta_adu))
    sol_1 <- nleqslv(beta_adu, eq_1, y = y_resp)
    
    y_resp_2 <- c(fit(lower, beta_adu), fit(upper, beta_adu) - eps_H)
    ...
    result_final <- filter(result, abs(max_difference - eps_H) < tol)
    ...
    beta_matrix <- rbind(beta_10, beta_15, beta_25, beta_35, beta_45, beta_55,
                         beta_65, beta_75, beta_85, beta_95)
    
    coefficients_list[[as.character(eps_H)]] <- beta_matrix
  }
...
lower=2.5
upper=5
result_typeI <- typeI_coeff(eps_H,n_adu,lower,upper,beta_adu=c(-2.830380,1.412872))
